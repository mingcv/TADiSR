    ...
    def compute_global_heat_map(self, prompt=None, factors=None, head_idx=None, layer_idx=None, normalize=False):
        ...
        with auto_autocast(dtype=torch.float32):
            for (factor, layer, head), heat_map in heat_maps:
                if factor in factors and (head_idx is None or head_idx == head) and (
            ...
            # maps = maps.mean(0)[:, 0]
            # 77,1,550,64,64
            maps = maps.permute(1, 2, 0, 3, 4).contiguous()
            # 12,1,550,64,64
            maps = maps[:len(self.pipe.tokenizer.tokenize(prompt)) + 2]  # 1 for SOS and 1 for padding

            if normalize:
                maps = maps / (
                        maps[1:-1].sum(0, keepdim=True) + 1e-6)  # drop out [SOS] and [PAD] for proper probabilities
...
class UNetCrossAttentionHooker(ObjectHooker[Attention]):
    ...
    def __call__(
        ...
        # print(attention_probs.shape, self.context_size, factor, self.latent_hw)
        # skip if too large
