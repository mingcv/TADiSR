...
__all__ = ['set_seed', 'compute_token_merge_indices', 'plot_mask_heat_map', 'cached_nlp', 'cache_dir', 'auto_device',
           'auto_autocast']
...
def compute_token_merge_indices(tokenizer, prompt: str, word: str, word_idx: int = None, offset_idx: int = 0):
    ...
    if word_idx is None:
        word = word.lower()
        search_tokens = [x.replace('</w>', '') for x in
                         tokenizer.tokenize(word)]  # New tokenizer uses wordpiece markers.
...
def compute_token_merge_indices_kolors(tokenizer, prompt: str, word: str, word_idx: int = None, offset_idx: int = 0):
    merge_idxs = []
    tokens = tokenizer.tokenize(prompt.lower())
    ...
    if word_idx is None:
        word = word.lower()
        search_tokens = [x.replace('▁', '') for x in
                         tokenizer.tokenize(word)]  # New tokenizer uses wordpiece markers.
        start_indices = [x + offset_idx for x in range(len(tokens)) if
                         tokens[x:x + len(search_tokens)] == search_tokens]
