...
class DualStreamSepBlock(nn.Module):
    def __init__(self, *args):
        super(DualStreamSepBlock, self).__init__()
...
class MuGIBlockOri(nn.Module):
    def __init__(self, c):
        ...
        self.block1 = DualStreamSeq(
            ...
            DualStreamGate(),
            DualStreamSepBlock(CABlock(c)),
            DualStreamSepBlock(nn.Conv2d(c, c, 1))
        )
    ...
    def forward(self, inp_l, inp_r):
        ...
        x_skip, y_skip = inp_l + x * 0, inp_r + y * self.a_r
        x, y = self.block2(x_skip, y_skip)
        out_l, out_r = x_skip + x * 0, y_skip + y * self.b_r
        return out_l, out_r
...
class DSRNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, width=48, middle_blk_num=1,
        ...
        super().__init__()
        self.intro_x = nn.Conv2d(in_channels, width, 3, padding=1)
        self.intro_y = nn.Conv2d(1, width, 3, padding=1)

        self.ending_x = nn.Sequential(
            nn.Conv2d(width, 3, 3, padding=1),
            nn.Sigmoid()
        )
        self.ending_y = nn.Sequential(
    ...
    def forward(self, x, y):
        x, y = self.intro_x(x), self.intro_y(y)
        encs = []
        ...
        x, y = self.ending_x(x), self.ending_y(y)
        return x, y
